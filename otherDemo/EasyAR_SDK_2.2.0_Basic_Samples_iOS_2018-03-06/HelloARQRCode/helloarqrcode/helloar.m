...
BOOL start()
{
    ...
    status &= (qrcode_scanner != nil) && [qrcode_scanner start];
...
void render()
{
    ...
    if ([frame index] != previous_qrcode_index) {
        ...
        if (text != nil && ![text isEqualToString:@""]) {
            NSLog(@"got qrcode: %@", text);
            s_onAlert([NSString stringWithFormat:@"got qrcode: %@", text]);
        }
    }
}
